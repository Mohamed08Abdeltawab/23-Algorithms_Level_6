...
class RedBlackTree
{
    ...
    {
        //new root or sub root -> left
        Node left = node.Left; // Left child becomes the new root of the subtree

        //make node.left hold the right subtree of left(new root or sub root)
        node.Left = left.Right; // Move the right subtree of left to the left subtree of node

        //if there is a right subtree of left(new root or sub root) then set its parent to node
        if (node.Left != null)
            node.Left.Parent = node; // Set parent of the new left child

        // Connect new root with the grandparent
