    ...
    private void AutoComplete(AVLNode node, string prefix, List<string> results)
    {
        if (node != null)
        {
            if (node.Value.StartsWith(prefix, StringComparison.OrdinalIgnoreCase))
            {
                ...
                AutoComplete(node.Left, prefix, results);//start with left because is the samll 
                AutoComplete(node.Right, prefix, results);//after finish the small in left start in right 
            }
            else
            {
                ...
                if (string.Compare(prefix, node.Value, StringComparison.OrdinalIgnoreCase) < 0)
                    AutoComplete(node.Left, prefix, results);//if compare the nigative value start in small in 
                else
